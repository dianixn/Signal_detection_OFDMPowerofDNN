...
load('DNN_Classification_Trained.mat');
...
Num_of_QPSK_symbols_DNN = 2 * Num_of_symbols * Num_of_frame;
...
QPSK_signal = reshape(QPSK_symbol, Num_of_subcarriers, Frame_size);
...
Received_data_DNN = classify(DNN_Trained, DNN_feature_signal);
Received_data_DNN = grp2idx(Received_data_DNN) - 1;

DNN_Received_symbol = flip(rem(floor(Received_data_DNN * M .^ (1 - length(dataSym(1:2)) : 0)), M), 2)';
...
DNN_Received_data = de2bi(DNN_Received_symbol, 2);
DNN_Data_Received = reshape(DNN_Received_data, [], 1);
...
DNN_numErrs_sym(Frame, 1) = sum(sum(round(dataSym(1:2)) ~= round(DNN_Received_symbol)));
DNN_SER_in_frame(Frame, 1) = DNN_numErrs_sym(Frame, 1) / length(DNN_Received_symbol);

DNN_numErrs_bit(Frame, 1) = sum(sum(round(reshape(de2bi(dataSym(1:2), 2),[],1)) ~= round(DNN_Data_Received)));
